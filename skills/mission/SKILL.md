...
For an Execution ticket, “independently reviewable” includes an independent implementation review in fresh agent context. The implementation agent returns a candidate; the ticket remains Active until that review passes. This is acceptance evidence inside the Execution ticket, not the later use-case QA phase.
...
| Execution | Route implementation to `strategic-implementation` when installed, then route the candidate to `implementation-review` in fresh context inside the same Active ticket. Non-trivial code changes need both implementer evidence and an independent verdict against the approved design and contract. | Candidate code/evidence, then `Pass`, `Request changes`, or `Inconclusive`; only `Pass` makes the ticket eligible for human Review. |
| Validation | Use a separate ticket and fresh context to exercise accepted use cases through the project-appropriate QA method. Route to `use-case-qa` when installed. The project may provide a simulator, browser harness, API driver, staging environment, or another observable test seam; Mission must not assume one globally. | Per-use-case evidence and QA verdict; Mission Control accepts or rejects it. |
| Task | Perform bounded setup or mechanical work. | Observable completion or blocker. |

Routing is not progression. After a route returns, stop at the ticket checkpoint unless the still-Active Execution contract requires fresh-context `implementation-review`; that review is in-ticket completion evidence, not progression, and Mission routes it without another human authorization. After the reviewer returns, apply its verdict without activating another ticket or advancing a gate. No executor inherits authority beyond its bounded route.
