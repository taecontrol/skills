...
For implementation, do not present the human Review brief when only the implementation agent has finished. Keep the Execution ticket Active while `C0` receives a fresh independent full `implementation-review`; repairs normally return incrementally to that same reviewer. Start another fresh full review only for a recorded material risk, architecture, reviewer-availability, lineage, or evidence trigger. After `Pass`, present the ticket to Mission Control. A later use-case QA assignment is a separate Validation ticket whose method comes from project capabilities and whose cases trace to accepted design behavior.
