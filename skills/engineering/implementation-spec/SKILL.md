...
- **CRAP index during slices:** identify the existing command, analysis scope, and coverage-generation prerequisites. When a changed-code check exists, require a maximum CRAP score of `8` for new or modified production code in that slice. If only a full-project check exists, specify whether it runs per slice or at integration according to project requirements; state that changed-code checking is unavailable. Preserve mandatory project gates and never present partial coverage as valid input to a full-project check.
...
- **New products:** when no runnable product exists yet, assign launch and control verification to the first runnable slice. Make this limit explicit rather than claiming method feasibility has already been demonstrated.

## Lifecycle

Treat an accepted specification as implementation input, not an execution log. Discovery may change reversible local details without revising it. If implementation evidence invalidates an outcome, boundary, consequential obligation, slice handoff, or success criterion, revise only the affected contract and obtain human acceptance before dependent work continues. An explicit user instruction changing that contract already supplies acceptance for that change; carry it forward without requesting it again.
