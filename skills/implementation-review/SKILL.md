...
Run a focused check when the profile requires independent execution or saved evidence is insufficient. Use only the candidate's assigned workspace and owned or exclusively leased mutable resources. Declared immutable dependency commits, contracts, and evidence are valid inputs; undeclared cross-slice artifacts or shared mutable state make the result inconclusive. A broad green summary does not replace obligation-level proof. Judge behavior at a faithful seam, preserved identities and invariants, accepted degraded states, ownership and dependency direction, and proof that could expose the old behavior or a plausible defect. Reject retained tests that pass by construction, derive expectations through the production algorithm, or duplicate proof without protecting a distinct behavior or failure mode. Check that retained names and dependencies remain meaningful after disposable goal coordination is removed. Leave product journeys to Product Validator.
...
Return the result to the requesting lifecycle owner. Under `pursue-goal`, the Slice Owner routes `Pass` to Product Validator, `Repair` to Cleaner, and `Resynchronize` to the Coordinator; it resolves an `Inconclusive` result inside the accepted allocation or returns `Blocked`. During goal validation, the Goal Validation Owner routes `Pass` to Product Validator but reports `Repair`, `Resynchronize`, or unresolved `Inconclusive` to the Coordinator as a slice failure, `Resynchronize`, or `Blocked`. It does not dispatch Cleaner or a Slice Owner.
