...
2. Require one current coordination envelope and one bounded assignment. Read [the execution contract](references/execution-contract.md) for required identities, topology, and capabilities. Reject superseded identities instead of merging them.
...
- A Slice Owner runs its complete internal lifecycle. The goal Coordinator schedules and integrates slices but does not assign or supervise their internal roles.
...
Completion criterion: every mutable actor is attached to the workspace and lease named by its assignment, and no second owner manages the same lifecycle transition.

## Launch and supervise

1. Start one fresh target session when the role requires independence. Confirm that it is attached to the intended workspace before delivering work.
...
5. Accept exactly one terminal result for each attempt. Reject results with stale identities, the wrong candidate, missing independence, or undeclared workspace changes.
